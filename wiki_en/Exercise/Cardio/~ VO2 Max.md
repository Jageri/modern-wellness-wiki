# VO2 Max

**Mortality Impact**: One of the strongest single predictors of mortality; low VO2 max increases mortality by 200–400%
**Evidence Level**: ⭐⭐⭐⭐⭐ (Extensive prospective studies, strong causal relationship)
**Actionability**: High (can be significantly improved through training)

## Conclusion
VO2 max (maximal oxygen uptake) is the gold standard for measuring cardiorespiratory fitness and one of the strongest predictors of all-cause mortality. It has stronger predictive power than traditional risk factors such as blood pressure, cholesterol, and smoking. Good news: it can be significantly improved through training.

## Evidence
- Mandsager study (122,007 participants): Those in the lowest fitness group vs the highest had ~400% increased mortality
- Importance ranking: VO2 max > smoking > diabetes > coronary heart disease (by mortality hazard ratio)
- Each 1 MET increase (~3.5 mL/kg/min) reduces mortality by ~12–15%
- VO2 max naturally declines ~10% per decade, accelerating to 15% after age 50
- Good news: Even starting training in middle age can reverse 10–20 years of decline

## Do
1. **Measure your baseline**: Estimate VO2 max through exercise testing or smartwatch
2. **Zone 2 training**: 3–4 times per week, 45 minutes each (heart rate 70–85% of max heart rate)
3. **High-intensity interval training (HIIT)**: 1–2 times per week, 4 minutes high intensity + 4 minutes recovery, repeat 4–6 sets
4. **Targets**: Men >40 mL/kg/min, Women >35 mL/kg/min (for the 40s age group)
5. **Priority**: Improving VO2 max yields greater benefits than reducing body fat percentage

## References
- Mandsager K et al. Association of Cardiorespiratory Fitness With Long-term Mortality. JAMA Netw Open. 2018.
- Kodama S et al. Cardiorespiratory fitness as a quantitative predictor of all-cause mortality. JAMA. 2009.
